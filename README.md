...
🎨 프론트엔드 (React)
<details>
<summary><strong>메인 UI 및 페이지 구성</strong></summary>

📌 설명  

React 기반으로 전체 화면 구조를 설계하고  
메인 페이지, 종목 검색 결과, 상세 페이지 등  
서비스 전반의 UI 흐름을 구현했습니다.

- 공용 컴포넌트 분리(Header, Layout, Chart 영역 등)
...
📊 지수 그래프 시각화
<details>
<summary><strong>코스피 및 시장 지수 그래프</strong></summary>

📌 설명  

코스피 및 주요 지수 데이터를 기반으로  
시간 흐름에 따른 **지수 변화 그래프**를 시각화했습니다.

- 일/분 단위 시계열 데이터 처리
- 지수 변동에 따른 동적 렌더링
- 사용자 관점에서 직관적인 차트 UI 구성
...
⚡ 상세 페이지 실시간 차트
<details>
<summary><strong>종목 상세 실시간 주가 차트</strong></summary>

📌 설명  

종목 상세 페이지에서  
**실시간으로 변하는 주가 데이터를 차트에 즉시 반영**하도록 구현했습니다.

- WebSocket을 통한 실시간 데이터 수신
- 차트 데이터 누적 및 갱신 처리
- 페이지 이동 시 구독/해제 로직 적용
