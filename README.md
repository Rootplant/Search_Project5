...
- **실시간 주가 데이터 처리 구조**
  - 종목 기본 정보는 DB 기반 REST API로 초기 조회
  - 가격 변동 데이터는 WebSocket으로만 수신하여 **차트 갱신 시 DB 조회 제거**
  - 실시간 트래픽 환경에서도 DB 부하 없이 안정적인 시세 반영 구현

- **실시간 차트 성능 최적화**
  - 차트 상태와 UI 상태 분리 관리
  - WebSocket 수신 데이터 중 필요한 값만 업데이트하여 재렌더링 최소화

- **다중 종목 WebSocket 연결 관리**
  - 종목 코드 기준 구독/해제 로직 적용
  - 페이지 이동 시 불필요한 연결 유지 방지로 안정성 확보

- **지수 데이터 조회 성능 개선**
  - 외부 API 지수 데이터는 초기 저장 후 Redis 캐싱 중심 조회
  - 신규 데이터 발생 시에만 DB에 **증분 삽입**하여 과부하 최소화
